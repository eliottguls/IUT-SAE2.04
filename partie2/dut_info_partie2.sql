...
CREATE TABLE _etudiant_temp_inscription(
      ...
      mention_bac         VARCHAR(100),
      bourse_superieur    VARCHAR(200),
      CONSTRAINT PK_ETUDIANT_TEMP_INSCRIPTION PRIMARY KEY(code_nip, annee_inscription)
);
...
CREATE TABLE _resultat(
      ...
      moyenne                     VARCHAR(50),
      CONSTRAINT PK_RESULTAT PRIMARY KEY(id_module, annee_univ, num_semestre, code_nip));
...
ALTER TABLE _inscription
    ADD CONSTRAINT fk_inscription_semestre FOREIGN KEY (num_semestre, annee_univ)
        REFERENCES _semestre(num_semestre, annee_univ);
...
INSERT INTO partie2._candidat
  (SELECT id_individu, no_candidat, classement, boursier_lycee, profil_candidat, etablissement, dept_etablissement, ville_etablissement, niveau_etude, type_formation_prec, serie_prec, dominante_prec, specialite_prec, LV1, LV2
...
ALTER TABLE _resultat
  ADD CONSTRAINT Fk_resultat_semestre FOREIGN KEY (annee_univ, num_semestre)
    REFERENCES _semestre (annee_univ, num_semestre);
    

CREATE VIEW _nb_etudiants_tp AS (
  SELECT groupe_tp, annee_inscription as 	annee_scolaire, count(*) as nb_etu
  FROM partie2._inscription
  GROUP BY groupe_tp, annee_inscription);
  

CREATE VIEW _bac_s_mention AS (
  SELECT *
  FROM partie2._candidat c
  INNER JOIN partie2._etudiant e ON c.id_individu = e.id_individu
  WHERE serie_prec = 'S');
